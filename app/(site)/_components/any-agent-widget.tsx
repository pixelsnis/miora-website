...
function AgentBadge({
  ...
  label: string;
  className: string;
  driftRef: (node: HTMLSpanElement | null) => void;
}) {
  return (
...
export function AnyAgentWidget({ play }: { play: boolean }) {
  ...
  const badgeRefs = useRef<Record<AgentId, HTMLSpanElement | null>>({
    openai: null,
    claude: null,
    cursor: null,
  });
  const dotRefs = useRef<Record<AgentId, HTMLSpanElement | null>>({
    openai: null,
    claude: null,
    cursor: null,
  });
  ...
  useAnimationFrame((time) => {
    for (const agent of agents) {
      ...
      const badge = badgeRefs.current[agent.id];
      if (badge) {
    ...
    >
      ...
      {agents.map((agent) => (
        <AgentBadge
          ...
          driftRef={(node) => {
            badgeRefs.current[agent.id] = node;
          }}
        />
      ))}
    </div>
